...
void Shape::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget) {
    ...
    case ShapeType::Trapezoid: {
        ...
        qreal w = rect.width();
        qreal h = rect.height();
        QPointF p1(rect.left() + w * 0.2, rect.top());     // Верхняя левая точка
        QPointF p2(rect.right() - w * 0.2, rect.top());    // Верхняя правая точка
        QPointF p3(rect.right(), rect.bottom());           // Нижняя правая точка
        QPointF p4(rect.left(), rect.bottom());            // Нижняя левая точка
        QPolygonF poly;
        poly << p1 << p2 << p3 << p4;
